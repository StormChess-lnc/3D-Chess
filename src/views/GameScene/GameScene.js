...
/***********************************************************************************************/
    ...
    makeDeposit = async (roomId) => {
        let llgContract = await getContractWithSigner(llgContractAddress, llgContractABI);
        ...
        if (res.transactionHash) {
            let llgRewardContract = await getContractWithSigner(llgRewardContractAddress, llgRewardContractABI);
    ...
    getStartTimeOfDay = async () => {
        try {
            let llgRewardContract = getContractWithoutSigner(llgRewardContractAddress, llgRewardContractABI);
            ...
            let diffDays = (Date.now() - startTimeOfDay.toNumber()) / (24*60*60*1000); 
            if(diffDays >= 1) {
                window.localStorage.setItem("wins", 0);
                window.localStorage.setItem("chance", 1);
            }
        } catch (e) {
            console.error("Error getting start time of day:", e);
            // Set default value if contract call fails
            this.setState({
                startTimeOfDay: 0
            });
        }
    ...
    getTax = async () => {
        try {
            ...
            console.error("Error getting tax:", e);
            // Set default value if contract call fails
            this.setState({
                tax: 2
            })
    ...
    determineIfHasBonus = async () => {
        // Check if wallet is connected before trying to get contract
        if (!this.props.wallet && !this.state.wallet) {
            return;
        }
        try {
            let llgRewardContract = await getContractWithSigner(llgRewardContractAddress, llgRewardContractABI);
        } catch (e) {
            // Wallet not connected, skip bonus check
            return;
        }
